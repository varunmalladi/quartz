...
# Normal
For $X$ an affine variety, we say that it is normal if $K[X]$ is [[Integral element|integrally closed]] in its field of fractions.
# Properties
1. If $X$ is an affine variety, then $I(X)$ is prime.
2. Let $X$, $Y$ be affine varieties and $A(X)$, $A(Y)$ their [[Affine coordinate ring|coordinate rings]]. Then $X\cong Y$ iff $A(X)\cong A(Y)$.
