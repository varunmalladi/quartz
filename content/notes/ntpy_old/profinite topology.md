...
- A [topological group](ntpy/topological group.md) $G$ is called *residually finite* if it its profinite topology is Hausdorff. 
...
In fact, Galois groups are even stronger in their topology: they are [profinite group](ntpy/profinite group.md)s.
...
2. An equivalent bases is the one consisting of all left cosets of all subgroups of finite index.
	1. Hence in the profinite topology, every finite index subgroup is open. But this is not generally true for [profinite group](ntpy/profinite group.md)s. Namely, the topologies may not agree.

*Proofs*
**2**
The proposed basis contains the one in the definition, so it suffices to consider the other inclusion. Any subgroup contains a nonempty normal subgroup ([core](ntpy/Definitions/core.md)) $N\subset H\subset G$. The profinite topology is translation invariant, so for any $g\in H$ we can translate $N$ to an open set $gN$ containing $g$. Since we can do this for every $g\in H$, $H$ is the union of all such translated open sets, hence in the topology.
