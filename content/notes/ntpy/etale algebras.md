...
# Idea
Dual to [etale space](ntpy/etale space.md) of [scheme](ntpy/scheme.md)s.

Stable under [base change, ascent](ntpy/Key Ideas/base change, ascent.md).
...
\end{align}
$$
- the second line is [tensor product (commutative algebra)](ntpy/tensor product (commutative algebra).md) in "Of algebras", properties 1 and 2
- the third line is by the [Chinese Remainder Theorem](ntpy/Theorems/Ring Theory/Chinese Remainder Theorem.md), namely letting $f=\prod_i f_i$ be the irreducible factorization over $F'$.
...
- Proof: Let $\mathfrak{m}_1,\dots,\mathfrak{m}_n$ be maximal ideals of $A$. By [Chinese Remainder Theorem](ntpy/Theorems/Ring Theory/Chinese Remainder Theorem.md), $$0\to \bigcap_i\mathfrak{m}_i\to A\to\prod_i A/\mathfrak{m}_i\to 0$$ is exact. By lemma 1, we may assume the $\mathfrak{m}_i$ are all the maximal ideals. Then $\bigcap_i\mathfrak{m}_i=Nil(A)=0$ (the nilradical is 0 as there are no nilpotents). Thus $A$ is the product of fields.
