...
The Plucker embedding is the canonical injection $$\begin{gather}p:Gr(k,V)\hookrightarrow\mathbb{P}(\Lambda^kV)\\ W\mapsto [u_1\wedge\cdots u_k]\end{gather}.$$ One can show that the image of this map is an [algebraic variety](ntpy/Definitions/Algebraic geometry/Algebraic subset.md).

# Examples
1. ([Projective space](ntpy/Definitions/Algebraic geometry/Projective space.md)) 
...
2. ($k=2$, $n=4$)	
	...
	2. The [Plucker quadric](ntpy/Definitions/Algebraic geometry/Plucker quadric.md) $Q\subset\mathbb{P}^5$ is the smooth [quadric](ntpy/Definitions/Algebraic geometry/quadric.md) defined as $$Q=\{\omega\in\Lambda^2V\mid \omega\wedge\omega=0\}$$
