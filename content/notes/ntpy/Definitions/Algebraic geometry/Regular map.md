...
**Definition.** Let $V$ be an [algebraic subset](ntpy/Definitions/Algebraic geometry/Algebraic subset.md). A map $f:V\to K$ is called _regular_ if there exists $F\in K[x_1,\dots,x_n]$ such that $F\mid_V=f$.
...
# Variations
- [finite morphisms](ntpy/Definitions/Algebraic geometry/finite morphisms.md)
...
This may be exteded to a linear function into the [cotangent space](ntpy/Definitions/Algebraic geometry/(co)tangent space (of a variety).md): $$\begin{gather}\text{d}_p:K[U]\to (T_pU)^\ast\\ f\mapsto \text{d}_pf\end{gather}$$ which restricts to an isomorphism $$\text{d}_p:\mathfrak{m}_p/\mathfrak{m}^2_p\overset{\sim}{\longrightarrow}(T_pU)^\ast$$ (see that entry for more).
