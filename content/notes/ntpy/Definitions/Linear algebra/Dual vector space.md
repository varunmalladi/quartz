...
### Transpose map
Recall from above that a map $f:V\to W$ induces a map $f^\ast:W^\ast\to V^\ast$. This extends to a contravariant functor from $\text{Vect}_F$ to its dual. In particular, this functor is the [representable functor](ntpy/Definitions/Category theory/Representable functor.md) represented by $F$:
$$
